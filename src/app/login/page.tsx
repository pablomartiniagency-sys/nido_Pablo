    ...
    <div className="min-h-screen bg-grid bg-glow flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-white">Acceder a Nido</h1>
          <p className="text-sm text-white/50 mt-1">Portal de gestión escolar</p>
        </div>

        <div className="glass rounded-2xl p-6 space-y-4">
          <div className="flex gap-1 bg-white/[0.03] rounded-lg p-1">
            <button onClick={() => setMode("login")}
              className={`flex-1 px-3 py-1.5 rounded-lg text-xs font-medium transition ${mode === "login" ? "bg-coral-500/20 text-coral-300 border border-coral-500/30" : "text-white/50 hover:text-white"}`}>
              Iniciar sesión
            </button>
            <button onClick={() => setMode("signup")}
              className={`flex-1 px-3 py-1.5 rounded-lg text-xs font-medium transition ${mode === "signup" ? "bg-coral-500/20 text-coral-300 border border-coral-500/30" : "text-white/50 hover:text-white"}`}>
              Crear cuenta
            </button>
          </div>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-[#131316] px-2 text-white/30">o</span></div>
          </div>
        ...
        <div className="text-center mt-6">
          <a href="/" className="text-xs text-white/30 hover:text-white/50">← Volver a Nido</a>
        </div>
      </div>
    </div>
  );
