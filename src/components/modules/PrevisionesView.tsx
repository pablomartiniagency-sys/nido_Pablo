...
type BalanceField = "caja" | "activosFijos" | "proveedores" | "adminPub";
...
export function PrevisionesView() {
  ...
  const getEffectiveBalance = useCallback(() => {
    ...
    return { caja, clientes, activosFijos, totalActivos, proveedores, adminPub, totalPasivos, patrimonioNeto };
  ...
  const startEditing = (field: BalanceField) => {
    ...
    setEditValue(String(balanceOverrides[field] ?? reporte.activos[field as keyof typeof reporte.activos] ?? reporte.pasivos[field as keyof typeof reporte.pasivos] ?? 0));
  };
      ...
      <div className="flex gap-1 bg-white/[0.03] rounded-xl p-1 w-fit">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-1.5 rounded-lg text-xs font-medium transition ${tab === t.key ? "bg-coral-500/20 text-coral-300 border border-coral-500/30" : "text-ink-500 hover:text-ink-900"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === "pyg" && (
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <Card className="p-4"><div className="label mb-1">Amortización</div><div className="text-lg font-bold text-ink-900">{eur(reporte.amortizacion)}</div></Card>
          ...
          <Card>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between py-3 font-semibold">
                ...
                <span className="text-emerald-400 text-lg">{eur(eff.totalActivos)}</span>
              </div>
            </div>
          </Card>
          <Card>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between py-3 border-b border-gray-200/60 font-semibold">
                ...
                <span className="text-red-400">{eur(eff.totalPasivos)}</span>
              </div>
              <div className="flex justify-between py-3 font-semibold">
                <span className="text-ink-900">Patrimonio neto</span>
                <span className={`text-lg ${eff.patrimonioNeto >= 0 ? "text-emerald-400" : "text-red-400"}`}>{eur(eff.patrimonioNeto)}</span>
              </div>
            </div>
          </Card>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {reporte.proyeccionTrimestral.map(p => (
              <Card key={p.trimestre} className="p-5">
                ...
                <div className="text-2xl font-bold text-ink-900 mb-3">{eur(p.ebitda)}</div>
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between"><span className="text-ink-500">Ingresos</span><span className="text-emerald-400">{eur(p.ingresos)}</span></div>
                  <div className="flex justify-between"><span className="text-ink-500">Gastos</span><span className="text-red-400">{eur(p.gastos)}</span></div>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-start gap-3 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                ...
                <div>
                  ...
                  <div className="text-ink-500">Los gastos de {reporte.gastosPorCategoria[0]?.categoria} representan {reporte.gastosPorCategoria[0] ? pct(reporte.gastosPorCategoria[0].pct) : "—"} del total. Revisa si hay margen de ahorro.</div>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 rounded-xl bg-blue-500/10 border border-blue-500/20">
                ...
                <div>
                  ...
                  <div className="text-ink-500">Agosto y diciembre suelen tener menor facturación. Planifica tus gastos fijos considerando estos meses valle.</div>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
                ...
                <div>
                  ...
                  <div className="text-ink-500">Tienes {eur(reporte.ingresosPendientes)} pendiente de cobro. Activa la remesa SEPA o revisa los plazos de pago.</div>
                </div>
              </div>
            </div>
          </Card>
